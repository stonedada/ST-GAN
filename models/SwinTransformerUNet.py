...
import copy
from util.config import get_device
...
class SwinTransformerUNetParallel(nn.Module):
    def __init__(self, channels: Tuple[int], num_heads: int = 2, image_size: int = 128, is_residual: bool = False,
        ...
        self.pos_embed_x = nn.ParameterList(
            [nn.Parameter(torch.randn(1, channels[i + 1], image_size // (2 ** i), image_size // (2 ** i)))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x + self.pos_embed_mhsa
        x = self.mhsa(x)

        for i, skip_x in enumerate(reversed(skip_x_list)):
            x = x + self.pos_embed_x[i]
            skip_x = skip_x + self.pos_embed_skip_x[i]
